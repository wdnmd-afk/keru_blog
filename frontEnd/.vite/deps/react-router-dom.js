import { require_react_dom } from './chunk-7XA4PLQE.js'
import { __toESM, require_react } from './chunk-BMKNQQUQ.js'
...
function createRouter(init) {
    ...
    async function handleLoaders(
        ...
        let shouldUpdateNavigationState =
            !isUninterruptedRevalidation && (!future.v7_partialHydration || !initialHydration)
        if (isFogOfWar) {
    ...
    async function startRedirectNavigation(request, redirect3, _temp2) {
        let { submission, fetcherSubmission, replace: replace2 } = _temp2 === void 0 ? {} : _temp2
        ...
        let redirectHistoryAction =
            replace2 === true || redirect3.response.headers.has('X-Remix-Replace') ? Action.Replace : Action.Push
    ...
    async function discoverRoutes(matches, pathname, signal) {
        ...
        while (true) {
            ...
            if (
                !newPartialMatches ||
                partialMatches.map(m => m.route.id).join('-') === newPartialMatches.map(m => m.route.id).join('-')
            ) {
                return {
                    type: 'success',
...
async function convertHandlerResultToDataResult(handlerResult) {
    ...
    if (type === ResultType.error) {
        if (isDataWithResponseInit(result)) {
            ...
            result = new ErrorResponseImpl(
                ((_result$init2 = result.init) == null ? void 0 : _result$init2.status) || 500,
                void 0,
                result.data
            )
        }
    ...
    if (isDeferredData(result)) {
        ...
        return {
            ...
            headers:
                ((_result$init4 = result.init) == null ? void 0 : _result$init4.headers) &&
...
function processRouteLoaderData(
    ...
    let loaderHeaders = {}
    let pendingError =
        pendingActionResult && isErrorResult(pendingActionResult[1]) ? pendingActionResult[1].error : void 0
...
var NavLink = React2.forwardRef(function NavLinkWithRef(_ref8, ref) {
    ...
    let isActive =
        ...
        (!end && locationPathname.startsWith(toPathname) && locationPathname.charAt(endSlashPosition) === '/')
    let isPending =
