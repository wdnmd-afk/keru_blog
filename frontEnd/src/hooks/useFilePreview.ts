...
export const useFilePreview = (options: UseFilePreviewOptions = {}) => {
     ...
     * 获取文件预览类型（带缓存）
     */
    const getPreviewTypeWithCache = useCallback(
        (fileName: string, mimeType?: string): FilePreviewType => {
     ...
     * 批量获取文件类型信息
     */
    const getMultipleFileTypeInfo = useCallback(
     ...
     * 检查文件是否可以预览
     */
    const checkFilePreviewable = useCallback(
    ...
    const computed = useMemo(
        () => ({
            ...
            // 是否可以直接预览
            canPreviewDirectly: state.currentFile ? canPreviewDirectly(state.previewType) : false,
