...
import type { FileItem, FileQuery, FileListResponse, UploadFileItem, FileInfo } from '@/types/files'
 ...
 * @returns 文件预览URL
 */
const getFilePreviewUrl = (filename: string): string => {
  // 优先使用环境变量
        ...
        set((state) => {
          ...
          if (file) {
            state.currentFileInfo = {
              url: getFilePreviewUrl(file.filename),
              name: file.filename,
