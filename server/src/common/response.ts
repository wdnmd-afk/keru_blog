// response.ts
import { BusinessException, ErrorCode, ErrorMessage } from './exceptions'
...
export class ApiResponse<T = any> {
  ...
  static error(
    ...
    return new ApiResponse(false, code, message || ErrorMessage[code], null, requestId)
  }
  ...
  static fromException(exception: BusinessException): ApiResponse<null> {
    return new ApiResponse(false, exception.code, exception.message, null, exception.requestId)
  }
...
export class Result<T = any> extends ApiResponse<T> {
  // 保持与原有Result类的兼容性
  static override success<T>(data?: T, message?: string): Result<T> {
    return super.success(data, message) as Result<T>
  }

  static override error(code: number, message: string): Result<null> {
  ...
  static override validationError(message: string): Result<null> {
