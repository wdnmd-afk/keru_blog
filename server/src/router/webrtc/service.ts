...
export class WebRTCService {
    ...
    async getRooms(query: RoomQueryDto): Promise<PaginatedResponseDto<RoomResponseDto>> {
        ...
        let filteredRooms = rooms.filter(room => {
            ...
            if (query.search && !room.name.toLowerCase().includes(query.search.toLowerCase())) return false
     ...
     * 加入房间
     */
    async joinRoom(roomId: string, data: JoinRoomDto & { userId: string }): Promise<{ connectionId: string }> {
        const room = this.rooms.get(roomId)
        if (!room) {
    ...
    async getUserStats(userId: string): Promise<any> {
        ...
        return {
            ...
            activeConnections: connections.filter(c => c!.connectionState === ConnectionState.CONNECTED).length,
            averageLatency: this.calculateAverage(connections.map(c => c!.latency)),
    ...
    async getServiceStatus(): Promise<ServiceStatusResponseDto> {
        ...
        const activeRooms = Array.from(this.rooms.values()).filter(r => r.status === RoomStatus.ACTIVE).length
        const activeConnections = Array.from(this.connections.values())
            .filter(c => c.connectionState === ConnectionState.CONNECTED).length
    ...
    private cleanupInactiveRooms(): void {
        ...
        for (const [roomId, room] of this.rooms) {
            if (now.getTime() - room.updatedAt.getTime() > inactiveThreshold && room.currentParticipants === 0) {
                this.rooms.delete(roomId)
