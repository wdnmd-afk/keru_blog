...
import { ConnectionState, IceCandidateDto, SessionDescriptionDto, SignalingMessageDto } from './dto'
...
export class WebRTCGateway {
  ...
  private setupSignalingHandlers(socket: Socket): void {
    ...
    socket.on(
      'ice-candidate',
      async (data: { candidate: IceCandidateDto; targetUserId?: string }) => {
        try {
          const user = this.connectedUsers.get(socket.id)
          if (!user || !user.roomId) {
            socket.emit('error', { message: '用户未在房间中' })
  ...
  getOnlineStats(): { totalUsers: number; roomStats: Record<string, number> } {
    ...
    for (const [_socketId, user] of this.connectedUsers) {
      if (user.roomId) {
