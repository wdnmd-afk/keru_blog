 ...
 */
export function validationMiddleware<T>(dtoClass: new () => T, skipMissingProperties = false) {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      if (errors.length > 0) {
        ...
        res.status(400).json(Result.validationError(errorMessages.join('; ')))
        return
      }
      ...
      res.status(500).json(Result.serverError())
      return
    }
  }
}
...
export function requireBody(req: Request, res: Response, next: NextFunction): void {
  if (!req.body || Object.keys(req.body).length === 0) {
    res.status(400).json(Result.validationError('请求体不能为空'))
    return
  }
  next()
  return
...
export function validateFileUpload(req: Request, res: Response, next: NextFunction): void {
  ...
  if (!file) {
    res.status(400).json(Result.validationError('没有上传文件'))
    return
  }

  // 使用配置文件中的文件大小限制
  if (UploadUtils.isFileSizeExceeded(file.size)) {
    res.status(400).json(Result.validationError(UploadUtils.getFileSizeErrorMessage()))
    return
  }
  ...
  if (!allowedMimeTypes.includes(file.mimetype)) {
    res.status(400).json(Result.validationError('不支持的文件类型'))
    return
  }
